...
module.exports.run = async (bot, client, config, message, command, args) => {
  try {
    ...
    if (!message.mentions.members.first().roles.has(config.trustedRoleID)) {
      message.mentions.members.first().roles.add(config.trustedRoleID, `${message.author.tag}: ${config.prefix}${command} ${args.join(' ')}`).catch(e => message.channel.send(`\`\`\`${e.stack}\`\`\``) && console.error(e.stack));
      message.react('✅').catch(e => message.channel.send(`\`\`\`${e.stack}\`\`\``) && console.error(e.stack));
    } else {
      message.mentions.members.first().roles.remove(config.trustedRoleID, `${message.author.tag}: ${config.prefix}${command} ${args.join(' ')}`).catch(e => message.channel.send(`\`\`\`${e.stack}\`\`\``) && console.error(e.stack));
      message.react('❌').catch(e => message.channel.send(`\`\`\`${e.stack}\`\`\``) && console.error(e.stack));
    }
  } catch (e) {
